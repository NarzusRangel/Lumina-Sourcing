...
function validateCodexIntegration(options = {}) {
  ...
  const sourceFiles = listMarkdownFiles(resolved.sourceAgentsDir);
  const sourceCount = sourceFiles.length;
  const sourceFileSet = new Set(sourceFiles);
  const codexAgentFiles = listMarkdownFiles(resolved.agentsDir);
  const codexAgentsCount = codexAgentFiles.filter((file) => sourceFileSet.has(file)).length;
  const codexRedirectsCount = codexAgentFiles.length - codexAgentsCount;
  ...
  return {
    ...
    metrics: {
      ...
      codexAgentFiles: codexAgentFiles.length,
      codexRedirects: codexRedirectsCount,
...
function formatHumanReport(result) {
  if (result.ok) {
    const lines = [
      `✅ Codex integration validation passed (agents: ${result.metrics.codexAgents}, skills: ${result.metrics.codexSkills}, redirects: ${result.metrics.codexRedirects})`,
    ];
